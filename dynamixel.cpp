...
const double pi = 3.14159265358979323846;  // Define pi manually
...
: Node("read_write_node")
{
  ...
  auto get_present_position_vec =
    ...
    std::shared_ptr<GetPositions::Response> response) -> void
    {
      ...
      response->position.resize(request->id.size());
      for(size_t i = 0; i< request->id.size(); i++){

        dxl_comm_result = packetHandler->read4ByteTxRx(
          portHandler,
          (uint8_t) request->id[i],
        ...
        double pi_response = ConvertEncodedInRad(present_position);
        response->position[i] = pi_response;
      }
    };

  get_position_server_vec_ = create_service<GetPositions>("get_position", get_present_position_vec);
}
...
double ReadWriteNode::ConvertEncodedInRad(uint32_t encoded){
  float angle = encoded*296/1023;
 ...
 */
  ...
  dxl_comm_result = packetHandler->write2ByteTxRx(
      portHandler,
      dxl_id,
      MOVING_SPEED,  // The address for the Moving Speed register
      speed,         // The desired speed value (0-1023 for AX-12A)
      &dxl_error     // Error byte to capture errors
  );

  // Check if the command was successful
  if (dxl_comm_result != COMM_SUCCESS) {
      std::cerr << "Failed to set moving speed: " << packetHandler->getTxRxResult(dxl_comm_result) << std::endl;
  } else if (dxl_error != 0) {
      std::cerr << "Dynamixel returned error: " << (int)dxl_error << std::endl;
  } else {
      std::cout << "Moving speed successfully set to " << speed << std::endl;
  }
